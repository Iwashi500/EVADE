...
extern void BottanSet(int id, BOTTAN bottan); //ボタンのセット
...
int SendRecvManager(void)
{
    ...
    if (select(gWidth, &readOK, NULL, NULL, NULL) < 0) //何も送られていなければここで止まる
    {
        /* エラーが起こった */
        printf("no send\n"); //チェック　全く起こっていなかった
        return endFlag;
    ...
    for (i = 0; i < gClientNum; i++)
    {
        if (FD_ISSET(gClients[i].fd, &readOK))
        {
            /* クライアントからデータが届いていた */
            /* コマンドを読み込む */
            RecvData(i, &id, sizeof(int));
            //終了コマンドではないのなら次のBOTTANデータを受け取る
            RecvData(i, &bottan, sizeof(BOTTAN));
            BottanSet(id, bottan); //ボタンの更新
        }
    }
    return endFlag;
}
